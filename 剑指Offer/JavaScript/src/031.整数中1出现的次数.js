function NumberOf1Between1AndN_Solution(n) {
    ...
    let result = 0;
    // 按照每一位数，规律总结
    for(let i = 1; i <= n; i *= 10) {   // 个位 十位 ... 逼近 n
        const diviver = i * 10;
        result += parseInt(n / diviver) * i + Math.min(Math.max(n % diviver - i + 1, 0), i);    // min(, i) 最大只能取到 i  // max(k, 0) 取到 0 说明 k < i
        // 等价
        // result += parseInt(n / diviver) * i;
        // const k = n % diviver;
        // if(k >= i * 2 - 1) {
        //     result += i;
        // } else if(i <= k && k < i * 2 - 1) {
